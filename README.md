...
- Optional fixed character name, or let the app pick one — names come from a **local bank of real, published name data** rather than being invented by the AI, which means no more invented-sounding names and no more getting the same name over and over. The AI still chooses which of the offered names fits the character's gender and background.
- **🎭 Cultural Default** (⚙️ Settings → Character Generation) — `Western` (the default: British Isles, continental Western/Northern Europe, North America, Australasia), `Anglo only`, or `Any culture`. Steers names, nationality and setting for every generation mode, including group cards, scenario NPCs, supporting cast and the idea sparks. **Your concept always wins**: ask for a Tokyo chef and you get a Tokyo chef. `Any culture` turns the steer off and restores AI-invented names drawn from traditions worldwide.
- **🎲 Name Generator** — pick origin, gender, character type, time period and optional guidance, then choose from 10 suggestions. For ordinary human characters in the eras the name data covers, the 10 names are drawn locally and appear instantly with no API call; non-human types (demon, alien, elf, fae, angel), eras outside the data (ancient, medieval, renaissance, far future, fantasy), free-text guidance and `Any culture` all fall back to the AI. Recently used names — and their first names and surnames separately — are remembered across sessions and shared with auto-naming, so the same names don't keep coming back.
