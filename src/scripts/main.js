  ...
  bindEvents() {
    ...
    if (webSearchSubmitBtn) {
      webSearchSubmitBtn.addEventListener("click", async () => {
        ...
        try {
          const res = await window.authFetch("/api/search/images", {
            method: "POST",
    ...
    // "Chat with Character" button — switches to the Roleplay Chat tab with
    // this character pre-attached (see handleChatWithChar in chat-ui.js).
    // The old standalone "Chat Tester" modal this used to open was removed
    // once Roleplay Chat fully replaced it — see git history if it's ever
